...
function leaves(node: SchemaNode, prefix = ""): Array<{ path: string; marked: boolean }> {
    return Object.entries(node.properties ?? {}).flatMap(([key, child]) => {
        ...
        return child.properties ? leaves(child, path) : [{ path, marked: child["x-mc-live-reload"] === true }];
    });
}
...
test("schema live marks equal the keys sampled by producer consumers and published JSON schema", async () => {
    ...
    const published = await Bun.file(new URL("../../../../assets/magic-context.schema.json", import.meta.url)).json() as SchemaNode;
    const generatedLive = leaves(generated).filter((row) => row.marked).map((row) => row.path).sort();
    const publishedLive = leaves(published).filter((row) => row.marked).map((row) => row.path).sort();
    ...
    const sampled = sampleLiveConfig(boot as MagicContextConfig, fresh as MagicContextConfig) as Record<string, unknown>;
