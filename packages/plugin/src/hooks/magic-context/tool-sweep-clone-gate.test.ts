...
import { closeDatabase, openDatabase, getOrCreateSessionMeta } from "../../features/magic-context/storage";
...
// Run bun test with --todo to execute the assertion that the clone retains its adoption marker.
test.todo("scoped gate adopted clone retains priced sweep policy", () => cloneFixture(true));

test.skipIf(!process.env.MC_GATE_OLD_ROOT)("scoped gate pre-fix reader tolerates reserved ledger entry", async () => {
    const root = process.env.MC_GATE_OLD_ROOT!;
    const { getMergedReasoningStrippedIds } = await import(`${root}/packages/plugin/src/features/magic-context/storage-meta-persisted.ts`);
    const { stripReasoningFromMergedAssistants } = await import(`${root}/packages/plugin/src/hooks/magic-context/strip-content.ts`);
    ...
    try {
        ...
        const messages = [
            { info: { id: "first", role: "assistant" }, parts: [{ type: "text", text: "text" }] },
            { info: { id: "second", role: "assistant" }, parts: [{ type: "reasoning", text: "thinking" }] },
        ];
        const before = structuredClone(messages);
        expect(stripReasoningFromMergedAssistants(messages, "anthropic", { frozenMessageIds })).toBe(0);
        expect(messages).toEqual(before);
        console.log(`SCOPED_GATE pre-fix reader entries=${JSON.stringify([...frozenMessageIds])} stripped=0`);
    } finally {
        closeDatabase();
        if (old === undefined) delete process.env.XDG_DATA_HOME;
        else process.env.XDG_DATA_HOME = old;
