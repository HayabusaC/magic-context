...
it("priced capture plus the injected system produces an unscaled correlated P/L sample", async () => {
    useTempDataHome("calibration-system-");
    const sessionId = "calibration-system-correlation";
    const { calibrationCandidates } = await import("./calibration-candidate");
    const { capturePricedCalibration } = await import("./calibration-observation");
    const { estimateFinalWireInputTokens } = await import("./final-wire-token-estimate");
    const { estimateTokens } = await import("./read-session-formatting");
    const { recordToolDefinition, getMeasuredToolDefinitionTokens } = await import(
        "../../features/magic-context/tool-definition-tokens"
    );
    const model = { providerID: "anthropic", modelID: "claude-fable-5-1" };
    recordToolDefinition(model.providerID, model.modelID, "calibration", "read", "Read", {
        type: "object",
    });
    const messages = [
        {
            info: { id: "user-calibration", role: "user" },
    ...
    capturePricedCalibration(sessionId, "anthropic/claude-fable-5-1", messages, estimate);
    const { handler } = buildHandler({ resolveModel: () => model });
    const output = { system: ["You are a helpful assistant."] };
    await handler({ sessionID: sessionId, model }, output);
    const local =
        estimateTokens(output.system.join("\n")) +
        estimateTokens("Hello") +
        getMeasuredToolDefinitionTokens(model.providerID, model.modelID, "calibration")!;
    const result = calibrationCandidates.complete({
        harness: "opencode",
        sessionId,
        modelKey: "anthropic/claude-fable-5-1",
        requestId: "user-calibration",
        responseId: "assistant-calibration",
        providerInput: local * 1.5,
        completedAt: 1,
    });
    expect(result).toMatchObject({ n: 1, sample: 1.5, ema: 1.5, completeness: "complete" });
});
