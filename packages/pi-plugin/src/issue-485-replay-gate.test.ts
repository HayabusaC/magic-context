...
import { clearContextHandlerSession, registerPiContextHandler } from "./context-handler";
import {
    assistantMessage,
    createFakePi,
    createTestDb,
...
it.skipIf(!process.env.MC_GATE_PURE)("F pure replay child", async () => {
    ...
    try {
        ...
        const raw = [userMessage("old", 1), assistantMessage("answer", 2), userMessage("tail", 3)];
        const passes = [];
        for (let pass = 0; pass < 6; pass++) {
            ...
            const served = await handler(
                { messages },
                {
                    ...fakeContext(sid, process.env.MC_GATE_EMPTY, ["u0", "a0", "u1"], messages),
                    getContextUsage: () => ({
            ...
            passes.push({
                pass,
                sha256: createHash("sha256").update(JSON.stringify(served.messages)).digest("hex"),
...
it("F no-system served arrays equal pre-fix master on every replay", () => {
    ...
    try {
        ...
        const archive = execFileSync("git", ["archive", baseline, "packages/pi-plugin"], {
            cwd: root,
        ...
        for (const pkg of ["plugin", "retina-local-fs"])
            symlinkSync(join(root, "packages", pkg), join(temp, "packages", pkg), "dir");
        symlinkSync(
            join(root, "packages/pi-plugin/node_modules"),
            join(temp, "packages/pi-plugin/node_modules"),
            "dir",
        );
        const script = join(temp, "packages/pi-plugin/src/issue-485-replay-gate.test.ts");
