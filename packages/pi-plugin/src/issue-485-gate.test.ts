...
import { __test as cloneTest, handlePiCloneSessionStart } from "./clone-inheritance";
import {
    ...
    return (tokens = 10000, runtime: unknown = manager) =>
        handler(
            { messages: structuredClone(manager.buildSessionContext().messages) },
            {
                ...
                ui: { notify() {} },
                model: { provider: "anthropic", id: "claude-sonnet-4-5", contextWindow: 100000 },
                sessionManager: runtime,
                getContextUsage: () => ({ tokens, percent: tokens / 1000, contextWindow: 100000 }),
            },
        );
}
function publish(
...
it("A C D reporter fold, process restart, LKG and second cut", async () => {
    ...
    try {
        ...
        updateSessionMeta(db, sid, { lastContextPercentage: 10, lastInputTokens: 10000 });
        let served: unknown[] = [];
        for (let i = 0; i < 4; i++) {
        ...
        if (replay.ok) expect(capture("A-LKG", replay.messages).sha256).toBe(folded.sha256);
        ...
        const secondBefore = capture("CD-before", manager.buildSessionContext().messages);
        ...
        updateSessionMeta(db, sid, { lastContextPercentage: 10, lastInputTokens: 10000 });
        expect(capture("CD-defer", (await pass()).messages).sha256).toBe(second.sha256);
    } finally {
...
for (const order of ["inject-first", "marker-first"])
    it(`B snapshot excludes MC prefix ${order}`, () => {
        ...
        try {
            ...
            const inject = () =>
                injectM0M1Pi(
                    { sessionId: sid, projectDirectory: process.cwd(), projectIdentity: "gate" },
                    db,
                    messages as never,
                    manager
                        .getBranch()
...
it("E displaced system remains invisible to initial resolver on defer", () => {
    const db = createTestDb();
    ...
    try {
        ...
        injectM0M1Pi(
            ...
            db,
            messages as never,
            ["extension", "system", "tail"],
        );
        capture("E-displaced", messages);
        expect(getCurrentTools(messages as never)).toHaveLength(3);
        // A preceding extension user hides the system entry from Pi's index-zero-only resolver.
        expect(getInitialSystemMessage(messages as never)).toBeUndefined();
...
it("I foreign system role is protocol state and excluded mass", () => {
    const db = createTestDb();
    try {
        const foreign = { role: "system", content: "FOREIGN ".repeat(20000), timestamp: 0 };
        const messages = [foreign];
        const transcript = createPiTranscript(messages, "foreign");
        const tagger = createTagger();
        tagger.initFromDb("foreign", db);
        expect(tagTranscript("foreign", transcript, tagger, db).targets.size).toBe(0);
...
it("H clone filter preserves system ordinals without tag targets", () => {
    const db = createTestDb();
    try {
        const entries = [initial, userMessage("old", 1), delta, userMessage("tail", 3)].map(
            (message, i) => ({ type: "message", id: `e${i}`, message }),
        );
        ...
        copySessionStateForClone(db, "source", "fork", cloneTest.createCloneFilter(entries));
        expect(getCompartments(db, "fork")[0]?.endMessage).toBe(3);
        expect(convertEntriesToRawMessages(entries).map((m) => [m.id, m.ordinal])).toEqual([
            ["e0", 1],
            ["e1", 2],
            ["e2", 3],
            ["e3", 4],
        ...
        view.commit();
        expect(getTagsBySession(db, "fork").some((t) => /e[02]/.test(t.messageId))).toBe(false);
    ...
    it(`G emergency interleaved pairing ${api}`, async () => {
        ...
        try {
            ...
            const body = payload as {
                messages?: { content: { type: string; id?: string; tool_use_id?: string }[] }[];
            ...
            for (const item of body.input ?? []) {
                if (item.type === "function_call" && item.call_id) wireCalls.push(item.call_id);
            ...
            console.log(`GATE G-wire-${api} sha256=${digest([payload])} pairs=${wireCalls.length}`);
        } finally {
            db.close();
        }
    });
...
it("J populated stale-view snapshot is adopted without equivalence fence", async () => {
    ...
    try {
        ...
        await pass();
        publish(db, manager, old, tail);
        const assumed = capture("J-fence-before", manager.buildSessionContext().messages);
        const runtime = new Proxy(manager, {
            get(target, key) {
                if (key === "appendCompaction")
        ...
        // Record the missing equivalence fence: the marker is accepted despite changing the folded tools and prompt.
        expect(equivalent || markers.length === 0).toBe(false);
        expect(markers).toHaveLength(1);
        const snapshot = markers[0]?.systemMessage;
        expect(snapshot).toBeDefined();
        if (!snapshot) throw new Error("Missing populated snapshot");
        expect(getCurrentTools([snapshot]).map((t) => t.name)).toEqual(after.tools);
        expect(getCurrentSystemPrompt([snapshot])).toBe(after.prompt);
        expect(after.tools).toEqual(["edit", "bash"]);
