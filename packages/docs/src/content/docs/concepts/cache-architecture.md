...
Only **dashboard** memory edits bump the project memory epoch, forcing an immediate m[0] re-materialize. The dashboard is an external editor that can't otherwise signal a running session.
