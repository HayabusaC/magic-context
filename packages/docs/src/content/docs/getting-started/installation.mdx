...
- **OpenCode** (current version) — for OpenCode installs. On OpenCode 2, or when upgrading an existing install from OpenCode 1 to 2, read [OpenCode 2](/getting-started/opencode-2/) first
...
Setup adds the plugin to your `opencode.jsonc` and turns off compaction. This example is OpenCode 1; on OpenCode 2 the key is `plugins` (see [OpenCode 2](/getting-started/opencode-2/#install-on-opencode-2)):
...
If you cannot run the wizard, add this to `opencode.jsonc` (on OpenCode 2, write `plugins` instead of `plugin`):
