...
type RefReplay = {
	...
	commit: string;
	passes: ReplayPass[];
    priced?: { mode: string; hardMessagesSha256: string; hardHistorySha256: string; expectedLocalBudget: number; commonHistorySha256: string; tailMessagesSha256: string; expectedDropped: number[]; actualDropped: number[]; restarted: boolean };
};
...
async function createReplayHarness() {
    ...
    if (!tsOnly) {
        ...
        if (fixtureWorkdir) {
            if (resolve(fixtureWorkdir) !== join(dirname(REPO_ROOT), "fixture-work")) throw new Error("fixture workdir must be inside the throwaway comparison root");
            mkdirSync(join(fixtureWorkdir, ".cortexkit"), { recursive: true });
            const config = join(harness.env.workdir, ".cortexkit/magic-context.jsonc");
            if (existsSync(config)) copyFileSync(config, join(fixtureWorkdir, ".cortexkit/magic-context.jsonc"));
	...
	try {
		...
		config.provider[options.providerID] = config.provider["mock-anthropic"] ?? config.provider[options.providerID];
        const provider = config.provider[options.providerID];
        provider.models[options.modelID] = { ...provider.models["mock-sonnet"], id: options.modelID, name: `Mock ${options.modelID}` };
		...
		return {
            ...
            openFixtureWriter: () => { const path = harness.contextDbPath(); if (!existsSync(path)) throw new Error("isolated context database is not initialized"); return new Database(path); },
            readMessages: async (sessionId: string) => {
                const { createOpencodeClient } = await import("@opencode-ai/sdk");
                const result = await createOpencodeClient({ baseUrl: harness.opencode.url }).session.messages({ path: { id: sessionId }, query: { directory: harness.opencode.env.workdir }, throwOnError: true });
                return result.data ?? [];
            },
            restart: async () => { await harness.restart(); await configure(); },
			sendPrompt: (sessionId: string, text: string) =>
				harness.sendPrompt(sessionId, text, options),
            sendSeedPrompt: (sessionId: string, text: string) => harness.sendPrompt(sessionId, text, { ...options, modelID: "mock-sonnet" }),
            ...schedulerObserver(gateLog),
			dispose: async () => { await harness.dispose(); rmSync(gateLog, { force: true }); if (priorLog === undefined) delete process.env.MAGIC_CONTEXT_LOG_PATH; else process.env.MAGIC_CONTEXT_LOG_PATH = priorLog; },
...
async function captureCurrentCheckout(
    ...
    if (priced && neutral) {
        if (existsSync(join(REPO_ROOT, ".git")) || !valueAfter("--fixture-workdir")) throw new Error("neutral override is allowed only inside an extracted throwaway ref");
        const path = join(REPO_ROOT, "packages/plugin/src/hooks/magic-context/tokenizer-calibration.ts");
        const source = readFileSync(path, "utf8");
        const signature = /export function resolveModelCalibration\([\s\S]*?\): ModelCalibration \{/;
        if (!signature.test(source)) throw new Error("neutral override could not locate resolver");
        writeFileSync(path, source.replace(signature, (match) => `${match}\n    if (providerId === "anthropic" && modelId === "claude-fable-5-1") return { systemRatio: 1, toolsRatio: 1, proseRatio: 1 };`));
    }
	...
	try {
        ...
        for (let warm = 0; warm < 2; warm++) {
            harness.mock.setDefault({ text: `[[warm-answer-${warm}]]`, usage: { input_tokens: 1000, output_tokens: 10 } });
            await harness.sendPrompt(sessionId, `[[warm-generation-${warm}]]`);
        }
        const generation = db.prepare("SELECT cached_m0_bytes, cached_m0_materialized_at FROM session_meta WHERE session_id = ?").get(sessionId) as { cached_m0_bytes: Uint8Array | null; cached_m0_materialized_at: number };
        if (!generation?.cached_m0_bytes) throw new Error("warmup did not establish a cached m0 generation");
		for (let index = 0; index < 4; index += 1) {
            const cursor = "schedulerCursor" in harness ? harness.schedulerCursor(sessionId) : 0;
            ...
            const observations = "schedulerSince" in harness ? await harness.schedulerSince(sessionId, cursor) : [];
            if (observations.length === 0 || observations.some((d) => d !== "defer")) throw new Error(`expected a current defer observation, got ${observations}`);
            const decision = observations.at(-1);
            const current = db.prepare("SELECT cached_m0_bytes, cached_m0_materialized_at FROM session_meta WHERE session_id = ?").get(sessionId) as typeof generation;
            if (current.cached_m0_materialized_at !== generation.cached_m0_materialized_at || !current.cached_m0_bytes || !Buffer.from(current.cached_m0_bytes).equals(Buffer.from(generation.cached_m0_bytes))) throw new Error("defer changed the established cached generation");
			passes.push({
				pass: index + 1,
...
function captureRef(
	...
	const child = spawnSync(
		...
		[
			...
			...(tsOnly ? ["--ts-only"] : []),
            ...(priced ? ["--priced", "--expected-local-budget", neutral || slot === "left" ? "60000" : "38173", "--expected-tool-ratio", neutral || slot === "left" ? "1" : "1.551639"] : []),
            ...(neutral ? ["--neutral"] : []),
			"--fixture-workdir",
...
function compare(left: RefReplay, right: RefReplay): boolean {
    if (priced) {
        ...
        const hard = neutral ? left.priced.hardMessagesSha256 === right.priced.hardMessagesSha256 : left.priced.hardHistorySha256 !== right.priced.hardHistorySha256;
        const tail = left.priced.commonHistorySha256 === right.priced.commonHistorySha256;
        const defers = [left, right].every((r) => r.passes.length === 4 && r.passes.every((p) => p.decision === "defer" && p.historySha256 === r.priced!.commonHistorySha256));
        console.log(`PRICED_GATE hard=${hard} tail_m0_equal=${tail} four_defers_each=${defers}`);
...
async function capturePricedCheckout(harness: Awaited<ReturnType<typeof createReplayHarness>>, ref: string, commit: string): Promise<RefReplay> {
    if (!("readMessages" in harness) || !("restart" in harness) || !("openFixtureWriter" in harness)) throw new Error("priced comparison requires --ts-only");
    const { appendCompartments } = await import("../../plugin/src/features/magic-context/compartment-storage");
    const { queuePendingOp } = await import("../../plugin/src/features/magic-context/storage");
    const { renderDecayedCompartments } = await import("../../plugin/src/hooks/magic-context/decay-render");
    const sessionId = await harness.createSession();
    ...
    const raw = (await harness.readMessages(sessionId)).filter((m) => m.info.role === "user" || m.info.role === "assistant");
    if (raw.length !== 52) throw new Error(`expected 52 real seed messages, got ${raw.length}`);
    const filler = "summary code decision result ";
    const compartments = raw.map((message, i) => ({ sequence: i + 1, startMessage: i + 1, endMessage: i + 1, startMessageId: message.info.id, endMessageId: message.info.id, title: `Work arc ${i}`, content: "", p1: `P1_ROW_${i} ${filler.repeat(640)}`, p2: `P2_ROW_${i} ${filler.repeat(320)}`, p3: `P3_ROW_${i} ${filler.repeat(160)}`, p4: `P4_ROW_${i} ${filler.repeat(48)}`, importance: 50, legacy: 0 }));
    appendCompartments(db, sessionId, compartments);
    // Restart to load the seeded database rows; switching mock-sonnet to Fable then triggers normal model-change materialization.
    db.close();
    ...
    const latestDecision = () => db.prepare("SELECT decision, materialized, materialize_reason FROM transform_decisions WHERE session_id = ? ORDER BY ts_ms DESC LIMIT 1").get(sessionId) as { decision: string; materialized: number; materialize_reason: string } | null;
    const history = () => {
        const request = harness.mainRequests().at(-1);
        const texts = (request?.body.messages ?? []).flatMap((m) => typeof m.content === "string" ? [m.content] : Array.isArray(m.content) ? m.content.flatMap((p: unknown) => p && typeof p === "object" && typeof (p as { text?: unknown }).text === "string" ? [(p as { text: string }).text] : []) : []);
        const block = texts.join("\n").match(/<session-history>[\s\S]*?<\/session-history>/)?.[0];
        if (!block) throw new Error("actual provider capture has no history block");
        return block;
    };
    ...
    if (actualHardHistory !== expectedHistory) {
        const meta = db.prepare("SELECT cached_m0_upgrade_state, cached_m0_model_key, last_usage_context_limit FROM session_meta WHERE session_id = ?").get(sessionId);
        const markers = (text: string) => [...new Set(text.match(/P[1-4]_ROW_\d+/g) ?? [])].join(",");
        throw new Error(`HARD history differs from independent local allowance ${budget}: actual=${hash(actualHardHistory)} expected=${hash(expectedHistory)} decision=${JSON.stringify(latestDecision())} meta=${JSON.stringify(meta)} actualTiers=${markers(actualHardHistory)} expectedTiers=${markers(expectedHistory)}`);
    }
    if (latestDecision()?.materialized !== 1) throw new Error(`expected an existing HARD materialization edge: ${JSON.stringify(latestDecision())}`);
    const hardMessagesSha256 = hash(harness.lastMainWireSerialized("messages"));
    for (let i = 0; i < 8; i++) {
        harness.mock.script([
            { content: [{ type: "tool_use", id: `priced-tool-${i}`, name: "bash", input: { command: "python3 -c 'print(\"word \" * 999, end=\"\")'", description: "Synthetic calibration fixture" } }], stop_reason: "tool_use", usage },
            { text: `[[tool-complete-${i}]]`, usage },
        ]);
        await harness.sendPrompt(sessionId, `[[tool-fixture-${i}]]`);
    }
    const rows = db.prepare("SELECT tag_number, token_count FROM tags WHERE session_id = ? AND type = 'tool' AND message_id LIKE 'priced-tool-%' ORDER BY tag_number").all(sessionId) as Array<{ tag_number: number; token_count: number | null }>;
    if (rows.length !== 8 || rows.some((r) => !r.token_count || r.token_count <= 0)) throw new Error(`tool fixture was not observed completely: rows=${rows.length}`);
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (decisions.some((d) => d !== "defer")) throw new Error(`priced generation defer ${i + 1} was not a defer: ${decisions}`);
        const wire = harness.lastMainWireSerialized("messages");
        const historySha256 = hash(history());
        if (historySha256 !== hash(commonHistory)) throw new Error(`defer ${i + 1} changed cached m0`);
        passes.push({ pass: i + 1, decision: decisions.at(-1) ?? null, bytes: Buffer.byteLength(wire), sha256: hash(wire), systemSha256: hash(harness.lastMainWireSerialized("system" as "messages")), toolsSha256: hash(harness.lastMainWireSerialized("tools" as "messages")), historySha256 });
    }
    return { ref, commit, passes, priced: { mode: neutral ? "neutral" : "fable", hardMessagesSha256, hardHistorySha256: hash(actualHardHistory), expectedLocalBudget: budget, commonHistorySha256: hash(commonHistory), tailMessagesSha256, expectedDropped, actualDropped, restarted: true } };
    } finally { db.close(); }
}
...
function schedulerObserver(logPath: string) {
    ...
    return {
        ...
        schedulerSince: async (sessionId: string, cursor: number) => {
            ...
            if (!observed.length) throw new Error("no current transform scheduler observation");
            return observed.map((line) => line.match(/decision=(execute|defer)/)?.[1] ?? "unknown");
        },
    };
}
