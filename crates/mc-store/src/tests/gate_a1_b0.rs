...
fn chain_without(version: u32) -> Vec<Migration> {
    MIGRATIONS
        .iter()
        .copied()
...
fn gate_the_claim_queue_then_the_marker_land_in_order_on_a_populated_store() {
    ...
    // 58 next, through the real open path.
...
fn gate_a_store_that_applied_58_first_never_gets_57() {
    ...
    // A build carrying the marker migration but not the claim-queue one.
    let marker_only = open_sqlite(&descriptor).unwrap();
    register_era_scope_functions(&marker_only);
    let skewed = marker_only
        .migrate(NS, &chain_without(CLAIM_LANE_MIGRATION_VERSION))
        .unwrap();
    ...
    // The full chain arrives later. 57 is at or below the recorded maximum, so it
    // is skipped for good.
    let full = McStore::open(&descriptor).unwrap();
    assert_eq!(
        full.module_store_schema_version().unwrap(),
        SINGLE_STORE_MARKER_MIGRATION_VERSION
    );
    assert!(
        !table_exists(&full.inner, HISTORIAN_QUEUE_TABLE),
        "57 is permanently skipped once 58 is recorded"
    );

    let loaded = full.load("ses").unwrap();
    let mut meta = ModuleMeta::default();
    meta.historian.state = HistorianPhase::Firing;
    meta.historian.firing_seq = 1;
    full.commit("ses", loaded.row_version, &CoreState::default(), &meta)
        .unwrap();
    let error = full
        .publish_pending_historian_run(&NewHistorianPendingRun {
    ...
    assert!(
        ...
        "the failure has to name the missing table: {rendered}"
    );
}

/// A store carrying both migrations, met by a binary whose chain stops at 56:
...
/// are still readable.
#[test]
fn gate_a_store_at_58_is_served_by_a_binary_whose_chain_stops_at_56() {
    let dir = tempfile::tempdir().unwrap();
    let descriptor = descriptor(dir.path());
